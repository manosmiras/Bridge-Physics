...
static void Reach(int i, const vec3 &target, std::vector<Link> &const links) {
	...
	if (abs(cosAngle) < 1.0f) {
		...
		float angle = glm::angle(vLinkBaseToEndEffDirection, vLinkBaseToTargetDirection);
		// Turn into a Quat
		quat q1 = glm::angleAxis(angle, axis);
		// Multiply our current Quat with it
		q1 *= qCur;
		// Pull out the angle and axis components, set the link params

		links[i].m_axis = glm::axis(q1);
		links[i].m_angle = glm::angle(q1);
		// *********************************
	}
}
