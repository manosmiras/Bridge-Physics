...
std::vector<sBall> balls; // red balls
std::vector<sBall> balls2; // blue balls
std::vector<sBall> balls3; // green balls
...
bool load_content() {
  ...
  for (size_t i = 0; i < 10; i++)
  {
	...
	balls2.push_back({ dvec3(0), dvec3((i * 2) - 15.0,10 + i,5), dvec3((i * 2) - 15.0,10 + i,5) });

	balls3.push_back({ dvec3(0), dvec3((i * 2) - 15.0,10 + i,-5), dvec3((i * 2) - 15.0,10 + i,-5) });
  }
...
bool render() {
  for (size_t i = 0; i < balls.size(); i++)
  {
	...
	phys::DrawSphere(balls2[i].position, 1.0f, BLUE);

	phys::DrawSphere(balls3[i].position, 1.0f, GREEN);
  }
