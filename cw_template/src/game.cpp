...
void cShapeRenderer::Render() {
	const mat4 m = Ent_->GetTranform(); //const mat4 m = glm::translate(Ent_->GetPosition()) * mat4_cast(Ent_->GetRotation()) * mat4_cast(quat(Ent_->GetScale()));
  switch (shape) 
  {
  case SPHERE:
	if (Ent_->GetName() == "Particle")
		phys::DrawSphere(Ent_->GetPosition(), 0.5f, col_);
	else
		phys::DrawSphere(Ent_->GetPosition(), 2.0f, col_);
    break;
  case BOX:
